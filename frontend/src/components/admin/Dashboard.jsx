...
/* ── Íconos SVG simples para las acciones rápidas ───────── */
...
const Dashboard = () => {
  ...
  const KPIS = [
    {
      ...
      bg:    dark ? 'rgba(244,63,94,0.15)'  : '#fff1f2',
      clr:   '#f43f5e',
    },
    {
      emoji: '🏥',
      ...
      sub:   '3 Postas · 3 EMR',
      bg:    dark ? 'rgba(20,184,166,0.15)' : '#f0fdfa',
      clr:   '#14b8a6',
    },
    {
      emoji: '📰',
  ...
  const ACCESOS = [
    ...
    },
    {
      to:    '/admin/horarios',
      emoji: '🕐',
      bg:    dark ? 'rgba(245,158,11,0.2)' : '#fffbeb',
      label: 'Editar Horarios',
      desc:  'Actualizar boxes y lab',
    },
    {
      to:    '/admin/postas',
      emoji: '🏥',
      bg:    dark ? 'rgba(20,184,166,0.2)' : '#f0fdfa',
      label: 'Gestionar Postas',
      desc:  'Actualizar red rural',
    },
    {
      to:    '/admin/analiticas',
      emoji: '📈',
  ...
  const ACTIVIDAD = [
    {
      emoji: '📰',
      texto: 'Noticia "Campaña Cardiovascular" publicada',
      tiempo: 'Hace 2 horas',
      bg:    dark ? 'rgba(59,130,246,0.15)'  : '#eff6ff',
    },
    {
      emoji: '🕐',
      texto: 'Horario Box 3 actualizado',
      tiempo: 'Hace 5 horas',
      bg:    dark ? 'rgba(245,158,11,0.15)'  : '#fffbeb',
    },
    {
      emoji: '🏥',
      texto: 'Posta Pupuya: horario modificado',
      tiempo: 'Ayer 14:30',
      bg:    dark ? 'rgba(20,184,166,0.15)'  : '#f0fdfa',
    },
    {
      emoji: '👤',
      texto: 'Sesión iniciada por Administrador',
      tiempo: 'Hace 2 días',
      bg:    dark ? 'rgba(139,92,246,0.15)'  : '#f5f3ff',
    },
  ]

  /* ── Helper: estilo de card ─────────────────────────────── */
  const card = (extra = {}) => ({
    background:   C.card,
    borderRadius: '16px',
    ...
    <AdminLayout>
      ...
      <div style={{ marginBottom: '28px' }}>
        <h1 style={{
          ...
          color: C.textP, display: 'flex', alignItems: 'center', gap: '8px',
        }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                <p style={{
                  ...
                  textTransform: 'uppercase', letterSpacing: '0.06em', color: C.textS,
                }}>
                  {k.label}
                </p>
                <p style={{ margin: 0, fontSize: '34px', fontWeight: '800', color: C.textP, lineHeight: 1 }}>
                  {k.valor}
                </p>
                <p style={{ margin: '6px 0 0', fontSize: '12px', color: C.textM }}>
                  {k.sub}
                </p>
              </div>
              <div style={{
            ...
            {ACCESOS.map(({ to, emoji, bg, label, desc }) => (
              <Link
                ...
                onMouseEnter={e => {
                  ...
                  e.currentTarget.style.borderColor   = C.hoverB
            ...
            {ACTIVIDAD.map(({ emoji, texto, tiempo, bg }, i, arr) => (
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{
                    margin: 0, fontSize: '13px', color: C.textP,
                    overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
                  }}>
                    {texto}
                  </p>
                  <p style={{ margin: '2px 0 0', fontSize: '11px', color: C.textS }}>
                    {tiempo}
                  </p>
                </div>
              </div>
